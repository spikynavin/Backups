...
#define NfcDataPath "/home/root/NfcData"
...
void onTagArrival(nfc_tag_info_t *pTagInfo)
{
	...
	{	
		...
		system("cp /dev/null /home/root/NfcData");
		fd = open(NfcDataPath, O_WRONLY);
		write(fd,"NFC Tag Found\n",15);
		close(fd);
		...
		if(eDevState_PRESENT == g_DevState)
		{
			...
			if(eDevType_TAG == g_Dev_Type)
			{
				...
				switch (TagInfo.technology)
				{
					...
					{
						printf("		'Type Unknown'\n");
						fd = open(NfcDataPath, O_WRONLY|O_APPEND);
						write(fd,"Type: \"Type Unknown\"\n",24);
						close(fd);
					} break;
					...
					case TARGET_TYPE_MIFARE_CLASSIC:
					{
						...
						fd = open(NfcDataPath, O_WRONLY|O_APPEND);
						write(fd,"Type: \"Type A - Mifare Classic\"\n",35);
						close(fd);
					} break;
					case TARGET_TYPE_MIFARE_UL:
					{
						...
						fd = open(NfcDataPath, O_WRONLY|O_APPEND);
						write(fd,"Type: \"Type A - Mifare Ul\"\n",30);
						close(fd);
					} break;
					case TARGET_TYPE_KOVIO_BARCODE:
					{
						...
						fd = open(NfcDataPath, O_WRONLY|O_APPEND);
						write(fd,"Type: \"Type A - Kovio Barcode\"\n",34);
						close(fd);
					} break;
					case TARGET_TYPE_ISO14443_3A_3B:
					{
						printf("		'Type A/B'\n");
						fd = open(NfcDataPath, O_WRONLY|O_APPEND);
						write(fd,"Type: \"Type A/B\"\n",20);
						close(fd);
					} break;
					default:
					{
						printf("		'Type %d (Unknown or not supported)'\n", TagInfo.technology);
						file = fopen(NfcDataPath,"a");
						fprintf(file,"Type: \" %d (Unknown or not supported)\"\n",TagInfo.technology);
						fclose(file);
				...
				if((0x00 != TagInfo.uid_length) && (32 >= TagInfo.uid_length))
				{
					...
					for(i = 0x00; i < TagInfo.uid_length; i++)
					{
						...
						file = fopen(NfcDataPath,"a");
						fprintf(file,"%02X ", (unsigned char) TagInfo.uid[i]);
						fclose(file);
					}
					printf("\'\n");
					fd = open(NfcDataPath, O_WRONLY|O_APPEND);
