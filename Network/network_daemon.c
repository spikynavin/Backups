...
int wifi_up_down_status=0, ethernet_up_down_status=0, enable_gprs_status=0, enable_wifi_status=0,enable_eth_status=0,ntp_status=0,ping_count=0, csq_checking_count=0;
...
{
    //    system("killall pppd;killall gsmMuxd;");
    //    file_write("/sys/class/gpio/gpio42/value","0");
    //    file_write("/sys/class/gpio/gpio288/value","0");
    printf("Restarting GPRS\n");
    disable_gprs();
    usleep(1000000);
    enable_gprs();
    //    file_write("/sys/class/gpio/gpio288/value","1");
...
static void operator_check(void)
{
    ...
    if(status_sim==1)
    {
        ...
        //        }else{
        //            csq_checking_count++;
        //            if(csq_checking_count==5){
        //                csq_checking_count=0;
        //            }
        //        }
...
{    
    char *ping_cmd = (int*)malloc(128 * sizeof(int));
    char ipadd[16];
    if( access( "/usr/share/status/pingip", F_OK ) != -1 ) {
        // file exists
        FILE *fp;
        fp = fopen("/usr/share/status/pingip","r");
        fgets(ipadd, sizeof(ipadd), fp);
        fclose(fp);
        ...
        printf("IpAddress: %s\n",ipadd);
        if(isValidIpAddress(ipadd)==1){
            printf("Ip Address is Vaild\n");
            strcat(ping_cmd,"ping ");
            strcat(ping_cmd,ipadd);
            strcat(ping_cmd," -c 1 -w 3 >/dev/null");
            printf("ping string: %s\n", ping_cmd);
        } else {
            printf("Ip Address is Not Vaild\n");
            ping_cmd = "ping 8.8.4.4 -c 1 -w 3 >/dev/null";
        }
    } else {
        printf("No File\n");
        ping_cmd = "ping 8.8.4.4 -c 1 -w 3 >/dev/null";
    ...
    if(a==3)
    {
        ...
        else
        {
            ...
            ping_count++;
            if(ping_count>2){
                file_write("/opt/daemon_files/ping_status","g"); // Network Status for SDK
            }
            if(ping_count>8){
                printf("GPRS restart command sent\n");
                restart_pppd();
                ping_count=0;
            }
        }
...
*/
            ...
            if(system("pidof -x gsmMuxd > /dev/null")!=0)
            {
            ...
            else
            {
                ...
                if(status_sim==1)
                {
                    ...
                    */
                    ...
                    if(system("pidof -x pppd > /dev/null")!=0)
                    {
