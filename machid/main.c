    ...
    while(1)
    {
        ...
        if(bytes_read > 0)
        {
            ...
            if(read_buffer[0]=='~' && read_buffer[1]=='C' && read_buffer[2]=='!')
            {
                printf("~OK!\n");
                write(fd,"~OK!",4);
                system("echo 0 > /media/nand/MACHINEID_status");
                machineid_mode=1;
            }else if(read_buffer[0]=='~' && read_buffer[1]=='F' && read_buffer[2]=='!'){
                printf("~OK!\n");
                write(fd,"~OK!",4);
                system("echo 1 > /media/nand/MACHINEID_status");
                machineid_mode=2;
            }
            if(machineid_mode==1){
                if(read_buffer[0]=='~' && read_buffer[1]=='C' && read_buffer[2]=='!'){
//                    write(fd,"~OK!",4);
                ...
                else if(read_buffer[0]=='~' && read_buffer[1]=='M' && read_buffer[2]=='N' && read_buffer[3]=='~' && read_buffer[bytes_read-3]=='~' && read_buffer[bytes_read-2]=='!')
                {
                    ...
                    write(fd,"~ERROR~8~!",12);
                    printf("Writing Machine ID Error\n");
                }
            }else if(machineid_mode==2){
                if(read_buffer[0]=='~' && read_buffer[1]=='F' && read_buffer[2]=='!'){
//                    write(fd,"~OK!",4);
                    printf("Initialization Command\n");
                }else if(read_buffer[0]=='~' && read_buffer[1]=='D' && read_buffer[2]=='C' && read_buffer[3]=='~' && read_buffer[strlen(read_buffer)-2]=='!' && read_buffer[strlen(read_buffer)-3]=='~') {
                    printf("Creating Config File\n");
                    ...
                    char str1[16];
                    if(read_buffer[4]=='V'){
                        system("chattr -i /media/nand/MACHINEid.enc;openssl aes-256-cbc -d -a -in /media/nand/MACHINEid.enc -out /tmp/eeprom.log -k \"technovates\";chattr +i /media/nand/MACHINEid.enc");
                        int ret = word_search("/tmp/eeprom.log","751");
                        if(ret == 1) {
                            FILE *fp = fopen("/tmp/eeprom.log","r");
                            fscanf(fp,"%s",str1);
                        ...
                        /*-------------------------------------Deleting Machine ID----------------------------------------*/
                        printf("Deleting Machine id\n");
                        system("chattr -i /media/nand/MACHINEid.enc;rm /media/nand/MACHINEid.enc");

                        if( access( "/media/nand/MACHINEid.enc", F_OK ) != -1 ) {
                            // file exists
                            printf("MACHINEid File Exist Error\n");
                            write(fd,"~ERROR~2~!",10);
                        } else {
                            // file doesn't exist
                            printf("MACHINEid file deleted success\n");
                            write(fd,"~OK!",4);
                        }
                    }
                    system("rm /tmp/eeprom.log");
                    ...
                    char str1[16];
                    if(read_buffer[4]=='V'){
                        system("chattr -i /media/nand/MACHINEserial.enc;openssl aes-256-cbc -d -a -in /media/nand/MACHINEserial.enc -out /tmp/eeprom.log -k \"technovates\";chattr +i /media/nand/MACHINEserial.enc");
                        int ret = word_search("/tmp/eeprom.log","HHC");
                        if(ret == 1) {
                            FILE *fp = fopen("/tmp/eeprom.log","r");
                            fscanf(fp,"%s",str1);
                        ...
                        /*-------------------------------------Deleting Machine Model Number----------------------------------------*/
                        system("chattr -i /media/nand/MACHINEserial.enc;rm /media/nand/MACHINEserial.enc");

                        if( access( "/media/nand/MACHINEserial.enc", F_OK ) != -1 ) {
                            // file exists
                            printf("MACHINEserial File Exist Error\n");
                            write(fd,"~ERROR~2~!",10);
                        } else {
                            // file doesn't exist
                            printf("MACHINEserial file deleted success\n");
                            write(fd,"~OK!",4);
                        }
                    }
                    system("rm /tmp/eeprom.log");
                ...
                else if(read_buffer[0]=='~' && read_buffer[1]=='M' && read_buffer[2]=='I' && read_buffer[3]=='~'&& read_buffer[bytes_read-3]=='~' && read_buffer[bytes_read-2]=='!')
                {
                    ...
                    if(strlen(token) >= 9)
                    {
                        printf("Writing Machine ID using EEPROM Tool\n");
                        char str1[40]="echo \"";
                        char str2[]="\" > /media/nand/MACHINEid";
                        strcat(str1,token);
                        strcat(str1,str2);
                        system(str1);
                        printf("%s\n",str1);
                    ...
                    int ret = word_search("/media/nand/MACHINEid","751");
                    if(ret == 1) {
                        printf("Word found in log file\n");
                        system("openssl aes-256-cbc -a -salt -in /media/nand/MACHINEid -out /media/nand/MACHINEid.enc -k \"technovates\";chattr +i /media/nand/MACHINEid.enc");
                        system("rm /media/nand/MACHINEid");
                        system("rm /usr/share/status/EEPROM-data");
                        write(fd,"~OK!",4);
                    }else if(ret == -2 ){
                        printf("word not found in log file\n");
                ...
                else if(read_buffer[0]=='~' && read_buffer[1]=='M' && read_buffer[2]=='N' && read_buffer[3]=='~' && read_buffer[bytes_read-3]=='~' && read_buffer[bytes_read-2]=='!')
                {
                    ...
                    if(strlen(token) >= 7)
                    {
                        printf("Writing Model number using EEPROM Tool\n");
                        char str1[40]="echo \"";
                        char str2[]="\" > /media/nand/MACHINEserial";
                        strcat(str1,token);
                        strcat(str1,str2);
                        system(str1);
                        printf("%s\n",str1);
                    ...
                    int ret = word_search("/media/nand/MACHINEserial","HHC");
                    if(ret == 1) {
                        printf("Word found in log file\n");
                        system("openssl aes-256-cbc -a -salt -in /media/nand/MACHINEserial -out /media/nand/MACHINEserial.enc -k \"technovates\";chattr +i /media/nand/MACHINEserial.enc");
                        system("rm /media/nand/MACHINEserial");
                        write(fd,"~OK!",4);
                    }else if(ret == -2 ){
                        printf("Word not found in log file\n");
                        write(fd,"~ERROR~6~!",10);
