...
void handle_alarm( int Sig, siginfo_t *Info, void *Ptr )
{
    ...
    if(Info->si_value.sival_ptr == &tmid0){
        ...
        write(2, "tmid1\n", 6);
        //system("poweroff");
        system("/opt/power_standby.sh > /dev/null &");
        sleep(1);
        /************************Reinitialize timer*************************/
        int bdata, kfile;
        bfile = fopen("/usr/share/status/backlight_read_time","r");
        ...
        while(1){
            if((fd = open(HIDFILE, O_RDONLY)) == -1) {
                ...
                while(read(fd, &ie, sizeof(struct input_event))) {
                    ...
                    if(0>timer_settime(tmid0, 0, &(struct  itimerspec const){ .it_value={bdata,0} } , NULL) )
                        system(buf);
                    //  system("exec /bin/sh /usr/share/scripts/backlight 3 &");
...
int main(int argc, char *argv[])
{
    ...
    key_t key;
    char *shm, *s;
    int home_press=0,alt_press=0,f4_press=0,homekey_press=0;
    ...
    if(kdata==2)
    {
        ...
        while (1) {
            ...
            if (ev.type == EV_KEY && ev.value >= 0 && ev.value <= 2)
            {
                ...
                //		file_write("/usr/share/status/key_event", ev.code);
                FILE *kf;
                ...
                //****************************Detect Symbol Keys*********************************//
                //                if(((int)ev.code==64 && (int)ev.value==0) && home_press==1)
                //                {
                //                    //system("echo 1 > /usr/share/status/KEYPAD_symbol");
                //                }
                //****************************Detect Symbol Keys*********************************//
                //****************************Screen Lock****************************************//
                //                if(homekey_press==1 && alt_press==1 ){
                //                    system("touch /usr/share/status/SCREEN_timeout;/usr/share/scripts/Buzzer 2;");
                //                }
                //****************************Screen Lock****************************************//
            }
            *s++ = task_bar_status[0];
